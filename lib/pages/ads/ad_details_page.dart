...
class _AdDetailsPageState extends State<AdDetailsPage>
  ...
  Future<void> _deleteAd() async {
    ...
    try {
      final response = await ApiService.post('/ads/delete/${widget.ad.id}', {});
      ...
      if (response.statusCode == 200 ||
        ...
        messenger.showSnackBar(
          SnackBar(
            content: Text('Failed to delete ad (${response.statusCode})'),
            backgroundColor: appColors.red,
            behavior: SnackBarBehavior.floating,
            shape: RoundedRectangleBorder(
              borderRadius: BorderRadius.circular(10),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      // ── AppBar: kept CustomAppBar + added delete action ──
      appBar: AppBar(
        backgroundColor: appColors.surface,
        surfaceTintColor: Colors.transparent,
        elevation: 0,
  ...
  Widget _hourlyChart(List<AdHourly> data) {
    if (data.isEmpty) return _emptyCard('No hourly data available');
    final sorted = [...data]..sort((a, b) => a.hour.compareTo(b.hour));
    final maxVal = sorted
        .fold<int>(0, (m, e) => e.plays > m ? e.plays : m)
        .toDouble();
