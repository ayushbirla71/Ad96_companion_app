...
class _DeleteSchedulePageState extends State<DeleteSchedulePage> {
  ...
  dynamic get selectedGroup {
    if (selectedGroupId == null || selectedGroupId == 'all') {
      return null;
    }

    return widget.schedule.groups.firstWhere(
      (g) => g.groupId == selectedGroupId,
    );
  }

  List<String> getTimeRangeOptions() {
    if (selectedGroup == null) {
      return ['entire', 'custom'];
    }

    final startDate = _parseDate(selectedGroup.fromDate);

    final endDate = _parseDate(selectedGroup.toDate);

    final today = DateTime.now();

    final totalDays = selectedGroup.totalDays;
  ...
  Map<String, String> getDateRangeForTimeOption() {
    ...
    switch (selectedTimeRange) {
      case 'entire':
        return {
          'from': _fmt(_parseDate(selectedGroup.fromDate)),
          'to': _fmt(_parseDate(selectedGroup.toDate)),
        };
      case 'today':
        return {'from': _fmt(today), 'to': _fmt(today)};
  ...
  String getTimeRangeLabel(String value) {
    switch (value) {
      case 'entire':
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          'Select how you want to remove this schedule',
                          style: TextStyle(
                            color: appColors.textMuted,
                            fontSize: 11,
                          ),
            ...
            _sectionLabel('Select Device Group'),
            ...
            // ─────────────────────────
            _buildGroupTile(
              title: 'All Device Groups',
              subtitle: 'Delete from all groups',
              value: 'all',
            ),
            ...
            // GROUPS
            // ─────────────────────────
            ...widget.schedule.groups.map(
              (g) => _buildGroupTile(
                title: g.groupName,
                subtitle: '${g.fromDate} → ${g.toDate}',
                value: g.groupId,
              ),
            ),
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      '${schedule.adName} is scheduled in ${selectedGroup.groupName}',
                      style: TextStyle(
                        color: appColors.textSecondary,
                        fontSize: 13,
                      ),
                    ...
                    Row(
                      children: [
                        ...
                        Text(
                          '${selectedGroup.fromDate} → ${selectedGroup.toDate}',
                          style: TextStyle(
                            color: appColors.textMuted,
                            fontSize: 12,
                          ),
            ...
            Container(
              ...
              child: Text(
                selectedTimeRange == 'entire'
                    ? 'This will completely remove the schedule.'
                    : selectedTimeRange == 'today'
                    ? 'This will remove the schedule only for today.'
                    : selectedTimeRange == 'week'
                    ? 'This will remove the schedule for this entire week.'
                    : selectedTimeRange == 'month'
                    ? 'This will remove the schedule for this entire month.'
                    : 'This will remove schedules between selected dates.',
                style: TextStyle(
                  color: appColors.textSecondary,
                  fontSize: 12,
                  height: 1.5,
  ...
  Widget _buildGroupTile({
    ...
    final isSelected = selectedGroupId == value;
